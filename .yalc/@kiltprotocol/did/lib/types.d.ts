...
import type { Deposit, IIdentity, IDidKeyDetails, IDidDetails, IDidServiceEndpoint, KeyRelationship } from '@kiltprotocol/types';
import { BN } from '@polkadot/util';
import type { AnyNumber } from '@polkadot/types/types';
import type { BTreeMap, BTreeSet, Enum, Option, Struct, Text, u64, u8, Vec } from '@polkadot/types';
import type { AccountId, BlockNumber, Call, Extrinsic, Hash } from '@polkadot/types/interfaces';
export declare type IDidParsingResult = {
    did: IDidDetails['did'];
    version: number;
    type: 'light' | 'full';
    identifier: string;
...
export declare type MapKeyToRelationship = Partial<Record<KeyRelationship, Array<IDidKeyDetails['id']>>>;
export interface INewPublicKey<T extends string = string> {
    publicKey: Uint8Array;
    type: T;
}
export declare type PublicKeyRoleAssignment = Partial<Record<KeyRelationship, INewPublicKey>>;
...
export declare type PublicKeyEnum = Record<string, Uint8Array>;
export declare type SignatureEnum = Record<string, Uint8Array>;
...
export declare type AuthenticationTxCreationInput = Omit<IAuthorizeCallOptions, 'blockNumber'> & {
    blockNumber?: AnyNumber;
};
declare type SupportedSignatureKeys = 'sr25519' | 'ed25519' | 'ecdsa';
declare type SupportedEncryptionKeys = 'x25519';
export interface DidVerificationKey<T extends string = SupportedSignatureKeys> extends Enum {
    type: T;
    value: Vec<u8>;
}
export interface DidEncryptionKey<T extends string = SupportedEncryptionKeys> extends Enum {
    type: T;
    value: Vec<u8>;
}
...
export declare type KeyId = Hash;
export declare type DidIdentifier = AccountId;
...
export declare type DidKeyAgreementKeys = BTreeSet<KeyId>;
export declare type DidPublicKeyMap = BTreeMap<KeyId, DidPublicKeyDetails>;
...
export declare type JsonDidDocument = {
    id: IDidDetails['did'];
...
export declare type JsonLDDidDocument = JsonDidDocument & {
    '@context': string[];
};
/**
 * The options that can be used to create a light DID.
 */
export declare type LightDidDetailsCreationOpts = {
    /**
...
export declare type FullDidDetailsCreationOpts = {
    did: string;
    keys: IDidKeyDetails[];
